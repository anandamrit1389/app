...
export default function useInjectScript(url: string): [boolean, boolean] {
  ...
  useEffect(() => {
    ...
    const onScriptEvent = (error: boolean) => {
      ...
      if (error) console.info('error loading the script');
    ...
    if (!injectorState.scriptMap?.[url]) {
      ...
      if (injectorState.scriptMap[url]) {
        ...
        injectorState.scriptMap[url].addEventListener('load', () => onScriptEvent(false));
        injectorState.scriptMap[url].addEventListener('error', () => onScriptEvent(true));
    ...
    return () => {
      ...
      injectorState.scriptMap[url]?.removeEventListener('load', () => onScriptEvent(true));
      injectorState.scriptMap[url]?.removeEventListener('error', () => onScriptEvent(true));
    };
  }, [url]);
